...
namespace AutoWeighbridgeSystem.ViewModels
{
    public partial class SettingsViewModel : ObservableObject
    {
        ...
        // RFID Driver Type
        [ObservableProperty] private string? _rfidDriverType;

        /// <summary>Danh sách loại driver RFID hỗ trợ — bind vào ComboBox.</summary>
        public IReadOnlyList<string> SupportedRfidDriverTypes { get; } =
            new List<string> { "SerialHf", "HopelandSdk" };

        // Google Sheets Settings
        [ObservableProperty] private string? _googleSheetsId;
        [ObservableProperty] private string? _googleSheetsName;
        [ObservableProperty] private bool _enableAutoSync = true;
        ...
        private void LoadConfig()
        {
            try
            {
                ...
                // General Settings
                StationName = _configuration["GeneralSettings:StationName"] ?? "KHOÁNG SẢN RẠNG ĐÔNG - ĐIỀU HÀNH TRẠM CÂN";
                ...
                // RFID Driver Type
                RfidDriverType = _configuration["RfidSettings:DriverType"] ?? "SerialHf";

                // Google Sheets
                GoogleSheetsId = _configuration["GoogleSheets:SpreadsheetId"];
                GoogleSheetsName = _configuration["GoogleSheets:SheetName"] ?? "HomNay";
                EnableAutoSync = bool.TryParse(_configuration["GoogleSheets:EnableAutoSync"], out bool autoSync) ? autoSync : true;
            }
            catch (Exception ex)
            {
        ...
        private async Task SaveSystemConfigAsync()
        {
            try
            {
                ...
                var connStr = (JsonObject)root["ConnectionStrings"]!;
                connStr["DefaultConnection"] = DbConnectionString;

                if (root["GeneralSettings"] == null) root["GeneralSettings"] = new JsonObject();
                var general = (JsonObject)root["GeneralSettings"]!;
                general["StationName"] = StationName;

                if (root["DatabaseSettings"] == null) root["DatabaseSettings"] = new JsonObject();
                var dbSettings = (JsonObject)root["DatabaseSettings"]!;
                dbSettings["BackupFolderPath"] = BackupFolderPath;

                if (root["CameraSettings"] == null) root["CameraSettings"] = new JsonObject();
                var cam = (JsonObject)root["CameraSettings"]!;
                cam["IpAddress"] = CamIpAddress;
                cam["RtspPort"]  = CamRtspPort;
                cam["Username"]  = CamUsername;
                cam["Password"]  = CamPassword;
                cam["RtspUrl"]   = CamRtspUrl;

                // ==========================================
                // 2. Scale Settings
                // ==========================================
                if (root["ScaleSettings"] == null) root["ScaleSettings"] = new JsonObject();
                var scale = (JsonObject)root["ScaleSettings"]!;
                ...
                var relay = (JsonObject)root["RelaySettings"]!;
                ...
                var signalLight = (JsonObject)root["SignalLightSettings"]!;
                ...
                var rfid = (JsonObject)root["RfidSettings"]!;

                if (rfid["ScaleIn"] == null)  rfid["ScaleIn"]  = new JsonObject();
                var rfidIn = (JsonObject)rfid["ScaleIn"]!;
                rfidIn["ComPort"]  = RfidInPort;
                rfidIn["BaudRate"] = RfidInBaudRate;
                rfidIn["DataBits"] = RfidInDataBits;
                rfidIn["Parity"]   = RfidInParity;
                rfidIn["StopBits"] = RfidInStopBits;

                if (rfid["ScaleOut"] == null) rfid["ScaleOut"] = new JsonObject();
                var rfidOut = (JsonObject)rfid["ScaleOut"]!;
                rfidOut["ComPort"]  = RfidOutPort;
                rfidOut["BaudRate"] = RfidOutBaudRate;
                rfidOut["DataBits"] = RfidOutDataBits;
                rfidOut["Parity"]   = RfidOutParity;
                rfidOut["StopBits"] = RfidOutStopBits;

                if (rfid["Desk"] == null) rfid["Desk"] = new JsonObject();
                var rfidDesk = (JsonObject)rfid["Desk"]!;
                rfidDesk["ComPort"]  = RfidDeskPort;
                rfidDesk["BaudRate"] = RfidDeskBaudRate;
                rfidDesk["DataBits"] = RfidDeskDataBits;
                rfidDesk["Parity"]   = RfidDeskParity;
                rfidDesk["StopBits"] = RfidDeskStopBits;

                rfid["DriverType"] = RfidDriverType;

                // ==========================================
                // 5. Google Sheets Settings
                // ==========================================
                if (root["GoogleSheets"] == null) root["GoogleSheets"] = new JsonObject();
                var googleSheets = (JsonObject)root["GoogleSheets"]!;
                googleSheets["SpreadsheetId"] = GoogleSheetsId;
                googleSheets["SheetName"] = GoogleSheetsName;
                googleSheets["EnableAutoSync"] = EnableAutoSync;
