...
namespace AutoWeighbridgeSystem.Services
{
    ...
    public class SignalLightService : IDisposable
    {
        ...
        /// </summary>
        public SignalLightService(IConfiguration configuration, IUserNotificationService notificationService)
        {
        ...
        public async Task TriggerLightAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                // JIT UI Notification
                _notificationService.ShowError("LỖI ĐÈN TÍN HIỆU: Không thể kích đèn qua mạch Relay!\nVui lòng kiểm tra cáp cắm hoặc nguồn điện.");
            }
            finally
            {
                _isLighting = false;
