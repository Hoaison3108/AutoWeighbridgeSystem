...
namespace AutoWeighbridgeSystem.Services
{
    ...
    public sealed class DashboardEventCoordinator : IDisposable
    {
        ...
        /// <summary>Cờ: đang có tải trọng hợp lệ đè lên cân (> MinWeightThreshold).</summary>
        private volatile bool _isScaleOccupied = false;
        ...
        /// <summary>Kích hoạt khi trạng thái Ngủ/Thức của hệ thống UHF RFID thay đổi.</summary>
        public event Action<bool>? RfidScanningStateChanged;
            ...
            Func<bool> getIsOnePassMode)
        {
            ...
            _rfidService.AutoReadersStateChanged  += isActive => Application.Current?.Dispatcher?.BeginInvoke(() => RfidScanningStateChanged?.Invoke(isActive));
        ...
        private void OnRfidCardRead(string readerRole, string cardId)
        {
            ...
            // Nhận được thẻ từ bất kỳ làn nào (kể cả Desk) -> tắt sóng ngay lập tức để ngủ theo yêu cầu
            Log.Information("[COORDINATOR] Đã nhận mã thẻ từ {Role} -> Tạm tắt sóng UHF chờ xe tiếp theo.", readerRole);
            _rfidService.SetAutoReadersActive(false);
